...
public class AvatarImageBehavior extends CoordinatorLayout.Behavior<CircularImageView> {
    ...
    public AvatarImageBehavior(Context context, AttributeSet attrs) {
        ...
        mFinalLeftAvatarPadding = context.getResources().getDimension(
                com.whiz.mobile.trackmed.R.dimen.spacing_normal);
    }
    ...
    private void bindDimensions() {
        mAvatarMaxSize = mContext.getResources().getDimension(com.whiz.mobile.trackmed.R.dimen.image_width);
    }
    ...
    private void shouldInitProperties(CircularImageView child, View dependency) {
        ...
        if (finalHeight == 0)
            finalHeight = mContext.getResources().getDimensionPixelOffset(com.whiz.mobile.trackmed.R.dimen.image_final_width);

        if (mStartXPosition == 0)
            mStartXPosition = (int) (child.getX() + (child.getWidth() / 2));

        if (mFinalXPosition == 0)
            mFinalXPosition = mContext.getResources().getDimensionPixelOffset(com.whiz.mobile.trackmed.R.dimen.abc_action_bar_content_inset_material) + (finalHeight / 2);

package com.jeonsoft.mobile.essbuddy.net;
        ...
        protected String doInBackground(String[]... params) {
            ...
            for (int i = 0; i < urls.length; i++) {
                try {
                    ...
                    if (callback != null)
                        callback.onStatusChanged(ServerHostStatus.Connecting, urls[i].trim());
                    ...
                    if (address != null) {
                        ...
                        if (nu.canConnect(address, port)) {
                            if (callback != null)
                                callback.onStatusChanged(ServerHostStatus.Connected, urls[i].trim());
                            return urls[i].trim();
                        } else {
                            if (callback != null)
                                callback.onStatusChanged(ServerHostStatus.CantConnect, urls[i].trim());
                        }
                    } else {
                        if (callback != null)
                            callback.onStatusChanged(ServerHostStatus.CantResolve, urls[i].trim());
                    }
                } catch(Exception ex) {
                    if (callback != null)
                        callback.onStatusChanged(ServerHostStatus.Unknown, "Error - " + ex.getMessage() + ": " + urls[i].trim());
                    continue;
                }
            }
            return null;
